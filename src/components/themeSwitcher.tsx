"use client";

import * as React from "react";
import { Moon, Sun, SunMoon } from "lucide-react";
import { useTheme } from "next-themes";

import { Button } from "@/components/ui/button";

export function ThemeSwitcher() {
  const { setTheme } = useTheme();

  return (
    <div className="flex justify-center" onClick={() => setTheme("light")}>
      <Button variant="outline" size="icon">
        <Sun className="h-[1.2rem] w-[1.2rem] rotate-0 scale-100 transition-all dark:-rotate-90 dark:scale-100" />
      </Button>{" "}
      <Button variant="outline" size="icon" onClick={() => setTheme("dark")}>
        <Moon className=" h-[1.2rem] w-[1.2rem] rotate-90 scale-100 transition-all dark:rotate-0 dark:scale-100" />
      </Button>{" "}
      <Button variant="outline" size="icon" onClick={() => setTheme("system")}>
        <SunMoon className=" h-[1.2rem] w-[1.2rem] rotate-90 scale-100 transition-all dark:rotate-0 dark:scale-100" />
      </Button>
    </div>
  );
}
